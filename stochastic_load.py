...
def also_x(profiles, eps=0.1, tol=10e-5, M=1e4, verbose=False):
    ...
    while True:
        ...
        prob = (1/(num_profiles*num_minutes))*np.sum(np.count_nonzero(y.value>=1e-7))
        """
        Alternatively, one might want to enforce that in 90% of scenarios, the capacity is always available
        prob =0
        for i in range(num_profiles):
            if not np.all(M*y.value[:,i]<=1e-2):
                prob+=1/num_profiles
        """
        print(prob)
        ...
        it_counter+=1
        if (q_overbar-q_underbar)<=tol:
            break

    return offer_capacity.value, problem, y.value
